...
public class NoteHighlighterManager : MonoBehaviour
{
    ...
    {
        if (_currentNoteSequence == null || _currentNoteSequence.Length == 0 || _hilighterCounter >= _currentNoteSequence.Length)
            return;
        ...
        foreach (GameObject key in _keys)
        {
            var keyComponent = key.GetComponent<Key>();
            if (keyComponent != null && keyComponent.keyName == currentNote)
            {
                _currentKey = key;
                break;
            }
        ...
        if (_currentKey == null)
        {
            Debug.LogWarning("Nota non trovata tra i tasti: " + currentNote);
            return; // Non continuare se non c'è un tasto valido
        }
        ...
        SetNextHighligher(); // Ora sicuro che _currentKey non è null
    }

    /*public void SetNextKey()
    {
        Debug.Log("c " + _currentNoteSequence); 
        Debug.Log("h " + _hilighterCounter);
